...
if __name__ == '__main__':
    """
    Trains CNN1D-RNN on MGB3 data
    """
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(levelname)s - %(message)s',
                        filename='fullmgb_cnn1d_rnn.log',
                        filemode='w+')
    ...
    Config.gru = True
    Config.architecture_type = 'cnn-rnn_1d'
    # audio_feat_schedule = [['mfcc'], ['logmel'], ['mfcc', 'dscc'], ['logmel', 'dscc']]
    # audio_concat = ['3d', 'append']
    audio_feat_schedule = [['logmel']]
    audio_concat = ['append']
    for ac in audio_concat:
        ...
        # Config.audio_features = audio_feat_schedule[1]
            ...
            test_dir = '/mnt/Shared/people/ryan/varDial2017_adi/test'
            ...
            train_x2, test_x, train_y2, test_y = process_files(test_dir, config=Config, speed=1.0, volume=1.0)
            test_x = train_x2 + test_x
            test_y = train_y2 + test_y
            ...
            model_dir = logdir('arabic_5sec',Config.architecture_type,Config.feature_concatenation,'-'.join(Config.audio_features),main_dir='fullmgb_models')
            ...
            # train_shapes = np.array([list(f.shape) for f in train_x])
            # test_shapes = np.array([list(f.shape) for f in test_x])
            # Config.inter_op_parallelism_threads = 1
            # Config.intra_op_parallelism_threads = 1
            # Config.device_count_cpu = 1
